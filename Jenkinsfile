             ...
             // echo $(cat /var/jenkins_home/workspace/Test_Pipeline/Script_Path.txt)
              ...
              script {
                ...
                if (env.FILENAME.contains('zip')) {
                 ...
                 sh "echo 'Hi Iam Here`cat Script_Path.txt`'"
                // echo 'http://100.111.89.228:3000/d/JMeter_Dashboard_Summary_NewDBSystem/jmeter_dashboard_summary_newdbsystem?orgId=1&from=now-15m&to=now&var-SessionName1="\$(cat Script_Path.txt)"'
                }
             }
